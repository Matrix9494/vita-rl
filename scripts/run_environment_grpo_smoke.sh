#!/usr/bin/env bash
# One selected environment -> Dressage -> Slime GRPO iteration on one H100.
#
...
RUN_ID="${RUN_ID:-environment-grpo-smoke-$(date -u +%Y%m%dT%H%M%SZ)}"
...
ENVIRONMENT="${ENVIRONMENT:-vitabench}"
ENVIRONMENT_HARNESS="${ENVIRONMENT_HARNESS:-vita_rl_standard}"
...
echo "[3/7] writing the environment prompt group"
if [[ "$ENVIRONMENT" == "vitabench" ]]; then
  cat >"$PROMPT_DATA" <<JSONL
{"prompt":"VitaBench delivery GRPO smoke", "label":"", "metadata":{"environment":"vitabench","task_id":"${ENVIRONMENT_TASK_ID}","agent_model":"proxy-model","max_steps":20,"environment_args":{"domain":"delivery","language":"chinese"},"reward_fn":"environment_smoke_dense"}}
JSONL
else
  cat >"$PROMPT_DATA" <<JSONL
{"prompt":"${ENVIRONMENT} GRPO smoke", "label":"", "metadata":{"environment":"${ENVIRONMENT}","task_id":"${ENVIRONMENT_TASK_ID}","agent_model":"proxy-model","max_steps":20,"environment_args":{"harness":"${ENVIRONMENT_HARNESS}"},"reward_fn":"environment_smoke_dense"}}
JSONL
fi

echo "[4/7] starting proxy and environment runtime"
...
RUNTIME_PYTHONPATH="$VITA_RL_ROOT/src:$VITA_RL_ROOT/external/vita-mini/src"
if [[ "$ENVIRONMENT" == "vitabench" ]]; then RUNTIME_PYTHONPATH="$RUNTIME_PYTHONPATH:$VITA_ROOT/src"; fi
PYTHONPATH="$RUNTIME_PYTHONPATH" VITA_MODEL_CONFIG_PATH="$VITA_MODEL_CONFIG" \
  env -u OPENROUTER_API_KEY "$VITA_VENV/bin/python" -m vita_rl.runtime_server --host 0.0.0.0 --port "$VITA_PORT" \
...
{"env_vars":{"PYTHONPATH":"/root/Megatron-LM:/root/Dressage:/root/Dressage/slime:$VITA_RL_ROOT/src:$VITA_RL_ROOT/external/vita-mini/src","DRESSAGE_PROXY_URL":"http://${MASTER_ADDR}:${PROXY_PORT}","ENVIRONMENT_RUNTIME_URL":"http://${MASTER_ADDR}:${VITA_PORT}","DRESSAGE_REWARD_MODULES":"vita_rl.reward","no_proxy":"127.0.0.1,localhost,${MASTER_ADDR}","CUDA_DEVICE_MAX_CONNECTIONS":"1"}}
JSON
)"
